...
def char_to_bit(plainText): #converts simple messages to bits
    bits = ''
...
def bit_to_char(result):  #converts bit to characters
    plainText = ''
...
def key_scheduling(array, key):  #key schedule function
...
def fill(array):  #the filling process
...
def cypher(key, plainText):    
    ...
    plain_bits = char_to_bit(plainText)
    
    group_en = group(plain_bits)
    group_en = fill(group_en)
